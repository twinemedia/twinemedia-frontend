...
export default {
    ...
    methods: {
        ...
        var tmpLists = []
        while(fetch) {
            var resp = await api.get(apiRoot+'lists', {
                offset,
                limit: fetchSize,
                type: 0,
                media: this.file
        ...
        async add(listId) {
            ...
            // Add item to list
            var resp = await api.post(apiRoot+'list/'+listId+'/add/'+this.file)
        ...
        async remove(listId) {
            ...
            // Add item to list
            var resp = await api.post(apiRoot+'list/'+listId+'/remove/'+this.file)
        ...
        async createList() {
            var name = this.newListName.trim()
            var input = this.$refs.listNameInput
            var submit = this.$refs.listSubmit
            ...
            if(name.length > 0) {
                ...
                try {
                    var res = await api.post(apiRoot+'lists/create', {
                        name,
                        type: 0,
                        visibility: 0
                    })
                    ...
                    if(res.status == 'success') {
                        // Add item to list
                        var resp = await api.post(apiRoot+'list/'+res.id+'/add/'+this.file)
