    ...
    <div class="files">
        <center v-if="$root.hasPermission('files.list')">
            ...
            <div class="results">
                ...
                <template v-else>
                    <h2>No Results</h2>
                    <p>Either you haven't uploaded any files, or your search returned no results</p>
                </template>
            </div>
        </center>
        <center v-else>
...
export default {
    ...
    methods: {
        ...
        async lastPage() {
            this.currentPage--
            if(this.$route.params.term)
                this.$router.push('/search/'+decodeURIComponent(this.searchTerm)+'/'+(this.currentPage+1))
            else
                await this.fetchFiles()
        },
        async nextPage() {
